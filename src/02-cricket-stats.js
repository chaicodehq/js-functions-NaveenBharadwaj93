...
export const calcBattingAvg = (totalRuns, innings, notOuts = 0) => {
  // Your code here
  const numInnings = innings - notOuts;
  if (numInnings <= 0) {
    return 0;
  }
  return Math.round((totalRuns / numInnings) * 100) / 100;
};
...
export const getPlayerCard = (player) => {
  ...
  const isAllRounderStatus = isAllRounder(battingAvg, economy);
  ...
  return {
    ...
    economy,
    battingAvg,
    isAllRounder: isAllRounderStatus,
  };
};
